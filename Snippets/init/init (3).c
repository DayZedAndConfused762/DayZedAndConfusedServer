...
class CustomMission: MissionServer
{
    ...
    {
		// Example with Bohemia UID: 	adminMembers.Insert( "Kvw3kCMuHni4zfKlykDq_FRXth_OcsSo0pq5dtGMwUA=" );
		// Example with Steam64 ID: 	adminMembers.Insert( "76561199153368149" );
...
// Admin Loadout
	EntityAI adminLoadOut(PlayerBase player)
	{
		...
		rags.SetQuantity(4);
		ItemBase light = ItemBase.Cast(player.GetInventory().CreateInInventory("Flashlight"));
		...
		gun.GetInventory().CreateAttachment("AK_PlasticBttstck");						// Weapon Attachment (Buttstock)
		gun.GetInventory().CreateAttachment("AK_Suppressor");						// Weapon Attachment (Silencer)
		ItemBase gunlight = ItemBase.Cast(gun.GetInventory().CreateAttachment("UniversalLight"));			// Weapon Attachment (Universal Light)
		gunlight.GetInventory().CreateAttachment("Battery9V");						//     Add battery to Universal Light
		addMags(player, "Mag_AKM_30Rnd", 4);								// Magazines
		ItemBase melee = ItemBase.Cast(player.GetInventory().CreateInInventory(meleeArray.GetRandomElement()));	// Melee
...
// Non-VIP Loadout (Default Loadout)
	EntityAI randomLoadOut(PlayerBase player) 
	{
		...
		ref TStringArray glassesArray = {""};
		ref TStringArray shoesArray = {"CombatBoots_Grey","CombatBoots_Beige","CombatBoots_Brown","CombatBoots_Black","CombatBoots_Green"};	
		ref TStringArray maskArray = {"FaceCover_Improvised"};	
		ref TStringArray glovesArray = {"HandsCover_Improvised"};		
		ref TStringArray foodArray = {"CarpFilletMeat"};
		ref TStringArray drinkArray = {"SodaCan_Cola"};
		ref TStringArray backpackArray = {"ImprovisedBag"};
		ref TStringArray knifeArray  = {"HuntingKnife"};
		...
		player.GetInventory().CreateInInventory("ImprovisedFishingRod");	
		ItemBase rags = ItemBase.Cast(player.GetInventory().CreateInInventory("Rag"));
		rags.SetQuantity(4);
		ItemBase light = ItemBase.Cast(player.GetInventory().CreateInInventory("Flashlight"));
		light.GetInventory().CreateAttachment("Battery9V");	
